...
claude plugin add oracle/    # 발견 (What to build?)
claude plugin add atlas/     # 설계 (How to architect?)
claude plugin add forge/     # 실행 (How to ship?)
claude plugin add argus/     # 운영 (How to measure?)
claude plugin add muse/      # 학습 (How to learn?)
```

만들지 말지부터 고민이라면 → `hplan`을 먼저 설치하세요 (evidence + COGS 게이트).
어떤 에이전트를 만들지 아직 모르겠다면 → `oracle`을 설치하세요.
...
> **참고:** 벤치마크는 v0.4 (32개 스킬) 기준 측정값입니다. v0.5 (43개 스킬, hplan 포함) 기준 재측정은 차기 iteration에서 진행 예정입니다.

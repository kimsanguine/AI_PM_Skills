...
**라우팅 규칙:**
- 프롬프트 최적화 필요 → `deliver/prompt` (CRISP 프레임워크 적용)
- 컨텍스트 윈도우 예산 계획 필요 → `deliver/ctx-budget` (메모리 전략의 토큰 배분)
- PRD 공식 문서화 필요 → `deliver/prd` (Instruction → PRD 변환)

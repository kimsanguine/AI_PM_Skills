...
- 인스트럭션 7요소 설계 필요 → `deliver/instruction` 스킬로 라우팅 (프롬프트 구조화)
- 프롬프트 최적화 필요 → `deliver/prompt` 스킬로 라우팅 (CRISP 프레임워크 적용)
- 비용 시뮬레이션 필요 → `discover/cost-sim` 스킬로 라우팅 (Flash vs Pro 비용 분석)
...
| 설계 영역 | 참조 스킬 | 연결점 |
|-----------|----------|--------|
| 비용 추정 | `discover/cost-sim` | Gemini 모델 티어별 비용 모델링 |
| 인스트럭션 | `deliver/instruction` | 프롬프트 템플릿의 7요소 적용 |
| PRD | `deliver/prd` | 이미지 생성 에이전트 PRD 작성 |
| 해자 분석 | `architect/moat` | 축적된 프롬프트 템플릿이 경쟁 해자 |
| TK 추출 | `learn/pm-engine` | 사용자 스타일 선호도의 TK화 |

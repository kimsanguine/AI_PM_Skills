...
- 비용 시뮬레이션/분석 필요 → `discover/cost-sim` 스킬로 라우팅 (비용 KR의 베이스라인 추정)
- 신뢰성/SLO 설계 필요 → `measure/reliability` 스킬로 라우팅 (신뢰성 KR의 구체화)
- 비용 추적/모니터링 필요 → `measure/burn-rate` 스킬로 라우팅 (비용 KR 모니터링)

...
**이 스킬이 소유하지 않는 영역:**
- 7요소 Instruction 설계 전체 (→ `deliver/instruction`)
- 컨텍스트 윈도우 토큰 최적화 (→ `deliver/ctx-budget`)
- 판단 로직의 암묵지 추출 (→ `learn/pm-engine`)
...
**라우팅 규칙:**
- 7요소 Instruction 전체 설계 필요 → `deliver/instruction`
- 컨텍스트 윈도우 최적화 필요 → `deliver/ctx-budget`
- PM 암묵지 추출 → `learn/pm-engine`

...
- 7요소 Instruction 설계 필요 → `deliver/instruction` 스킬로 라우팅
- 컨텍스트 윈도우 최적화 필요 → `deliver/ctx-budget` 스킬로 라우팅 (토큰 절감)
- 판단 기준 구체화 필요 → `learn/pm-engine` 스킬로 라우팅 (PM 사고 엔진)
...
| 프롬프트가 너무 길어서 실제 작업 입력 토큰 부족 | 컨텍스트 윈도우의 50% 이상이 시스템 프롬프트 차지 | deliver/ctx-budget 스킬로 라우팅하여 압축 전략 수립 |

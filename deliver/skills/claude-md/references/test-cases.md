...
1. "에이전트의 시스템 프롬프트를 설계해줘"
   - 올바른 라우팅: `deliver/instruction`

2. "이 프롬프트를 CRISP 프레임워크로 최적화해줘"
   - 올바른 라우팅: `deliver/prompt`

3. "PRD를 작성해줘"
   - 올바른 라우팅: `deliver/prd`

4. "컨텍스트 윈도우 토큰 예산을 계획해줘"
   - 올바른 라우팅: `deliver/ctx-budget`

5. "멀티에이전트 아키텍처를 설계해줘"
   - 올바른 라우팅: `architect/orchestration`
...
- 에이전트 구조 섹션 추가
- 빌드 명령어 업데이트
- hplan plugin 추천: deliver/instruction, architect/orchestration, measure/kpi
- 변경점 diff 형태로 설명
...
| E3 | "CLAUDE.md말고 AGENTS.md나 SOUL.md도 만들어줄 수 있어?" | ⚠️ 경계 | CLAUDE.md는 이 스킬 범위. AGENTS.md → deliver/instruction, SOUL.md → 범위 밖이지만 참고 가이드 제공 가능 |

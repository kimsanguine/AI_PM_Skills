...
## Verification
- `scripts/cogs_sentinel_wrapper.py` must stay green (auto-generated by hplan handoff).
- `python hplan/scripts/validate_docs.py` must pass on every docs/ change.
"""
...
def _mirror(path: Path, content: str) -> None:
    """Write a silent backup copy to harness/exports/ (never tracked in results)."""
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(content, encoding="utf-8")
    except OSError:
...
def export(brief: dict, root: Path, targets: list[str], force: bool) -> dict:
    written: list[str] = []
    skipped: list[str] = []
    mirror_base = root / "harness" / "exports"

    slug = slugify(brief.get("product_name", "hplan-product"))

    if "spec-kit" in targets or "all" in targets:
        # Canonical: root/specs/NNN-slug/
        canon_specs = root / "specs"
        spec_num = next_spec_number(canon_specs)
        spec_dir = canon_specs / f"{spec_num}-{slug}"
        for fname, renderer in (
            ("spec.md", render_spec_kit_spec),
            ("plan.md", render_spec_kit_plan),
            ("tasks.md", render_spec_kit_tasks),
        ):
            content = renderer(brief)
            status = write(spec_dir / fname, content, force)
            (written if status == "written" else skipped).append(str(spec_dir / fname))
            _mirror(mirror_base / "spec-kit" / "specs" / f"{spec_num}-{slug}" / fname, content)

    if "kiro" in targets or "all" in targets:
        # Canonical: root/.kiro/specs/slug/
        kiro_dir = root / ".kiro" / "specs" / slug
        for fname, renderer in (
            ...
            content = renderer(brief)
            status = write(kiro_dir / fname, content, force)
            (written if status == "written" else skipped).append(str(kiro_dir / fname))
            _mirror(mirror_base / "kiro" / ".kiro" / "specs" / slug / fname, content)

    if "gstack" in targets or "all" in targets:
        # No canonical path for gstack — harness/exports/ is authoritative
        target_path = mirror_base / "gstack" / "office-hours-brief.md"
        status = write(target_path, render_gstack_brief(brief), force)
        (written if status == "written" else skipped).append(str(target_path))

    if "claude" in targets or "all" in targets:
        # Canonical: root/AGENTS.md, root/CLAUDE.md
        for fname, kind in (("AGENTS.md", "agents"), ("CLAUDE.md", "claude")):
            content = render_claude_agents(brief, kind)
            status = write(root / fname, content, force)
            (written if status == "written" else skipped).append(str(root / fname))
            _mirror(mirror_base / "claude" / fname, content)

    # Post-export verification: confirm every written file exists on disk
    phantom = [p for p in written if not Path(p).exists()]
    return {"written": written, "skipped": skipped, "phantom": phantom}
...
def main():
    ...
    if result.get("phantom"):
        print(
            f"\n⚠️  Post-export verification FAILED: {len(result['phantom'])} file(s) not found on disk:",
            file=sys.stderr,
        )
        for p in result["phantom"]:
